...
// Shared render + metadata for the visa pair pages. This module is imported
// by the two route entries that wrap it:
//   - /[passport]/[destination]            → bare tourism page (page.tsx)
//   - /[passport]/[destination]/[purpose]  → per-purpose page ([purpose]/page.tsx)
//
// CRITICAL (cost): this content reads NO per-request inputs — no searchParams,
// no cookies, no headers. The purpose is passed in as a plain argument derived
// from the URL PATH. That is what lets each page be statically rendered and
// reused (ISR) instead of rebuilt on every request. Reading searchParams or a
// cookie here would opt every one of the ~235k pair URLs back into per-request
// rendering — which is exactly what caused the 39× function-CPU spikes when a
// crawler walked the sitemap. Per-visitor variation (currency, language, the
// signed-in "watch" state) is resolved on the CLIENT after hydration.
...
export async function buildPairMetadata(
  passport: string,
  destination: string,
  purpose: Purpose,
): Promise<Metadata> {
...
export async function PairContent({
  passport,
  destination,
  purpose,
}: {
  ...
  // Language is fixed to English in the server render so the page stays
  // statically cacheable. The header language toggle (a client component)
  // re-renders translatable strings on the client after hydration; the
  // <Money> component likewise reads the vl_currency cookie at hydration
  // time. No per-request inputs are read here — see the module header.
  ...
  try {
    ...
    // NOTE: the page already merges options across EVERY purpose above (primary
    // + all alternatives), so the result list is comprehensive regardless of
    // the URL's purpose. Profile-specific personalisation (e.g. "Doctor"
    // surfacing skilled-migration routes first) is handled client-side on the
    // destination pages, keeping this render free of per-request inputs.
  } catch (e) {
