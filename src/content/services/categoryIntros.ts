...
export const CATEGORY_INTROS: Record<ServiceCategory, CategoryIntro> = {
  travel_insurance: {
    ...
    howWePicked:
      "Selection criteria when provider listings return: (a) reputable underwriter (avoid white-label resellers without a named insurer), (b) clear and accessible claims process — published email + UK/EU office hours rather than chatbot-only support, (c) coverage compliant with the most common visa-application requirements (Schengen €30,000 medical, evacuation included, civil-liability cover), and (d) transparent pricing without per-day surcharge hidden in the fine print. " +
      "Specific commercial provider listings are currently removed pending affiliate-partner agreements. See our Disclosure for the full commercial policy.",
    whenYouDontNeedThis:
  ...
  passport_photos: {
    ...
    howWePicked:
      "Selection criteria when provider listings return: specific-country templates (US visa, Schengen, UK, Canadian, Chinese, Indian, etc.) with explicit compliance guarantees — if a passport-issuing authority rejects the photo, the service regenerates at no charge. Transparent pricing (no surprise per-photo upcharges) and digital + printed delivery options. Specific commercial provider listings are currently removed pending affiliate-partner agreements.",
    whenYouDontNeedThis:
  ...
  legal_services: {
    ...
    howWePicked:
      "Selection criteria when provider listings return: (a) appropriate jurisdictional licensing for the visa categories they cover (US AILA, UK OISC, Canadian RCIC, Australian MARA), (b) clear specialism focus (we'd list family-visa lawyers separately from EB-5 specialists from cross-border tax accountants), (c) flat-fee transparency where possible, and (d) consistent client feedback over time. " +
      "Specific firm listings are currently removed pending disclosed referral relationships. We never accept money to rank a firm, and we don't list 'visa concierge' or generic 'relocation services' that lack professional licensing — those are typically white-label intermediaries.",
    whenYouDontNeedThis:
